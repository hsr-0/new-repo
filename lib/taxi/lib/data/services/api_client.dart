...
import 'package:cosmetic_store/taxi/lib/data/model/global/response_model/unverified_response_model.dart';
import 'package:cosmetic_store/taxi/lib/environment.dart';
import 'package:cosmetic_store/taxi/lib/data/services/local_storage_service.dart';
...
class ApiClient extends LocalStorageService {
    ...
    try {
      ...
      switch (method) {
        ...
        case Method.updateMethod:
          response = await _dio.patch(requestUrl);
          break;
        default: // GET
          if (passHeader && !isOnlyAcceptType) {
            _dio.options.headers["Authorization"] = "$tokenType $token";
  ...
  Future<ResponseModel> multipartRequest(
    ...
    try {
      ...
      switch (method) {
        case Method.postMethod:
          response = await _dio.post(requestUrl, data: formData);
          break;
        case Method.updateMethod:
          response = await _dio.patch(requestUrl, data: formData);
          break;
        default:
