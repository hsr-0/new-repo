...
class _StoreLoginScreenState extends State<StoreLoginScreen> {
  ...
  Future<void> _getCurrentLocation() async {
    ...
    try {
      ...
      if (!hasPermission) {
        setState(() => _locationStatus = 'الوصول للموقع مرفوض');
        return;
      }
      ...
      if (mounted) setState(() => _locationStatus = 'فشل تحديد الموقع');
    }
  }

  // 🔐 دالة الدخول (نفس منطق المطعم)
  Future<void> _login() async {
    ...
    try {
      // 2. الاتصال بالبروفايدر (نستخدم listen: false لتجنب الخطأ)
      ...
      if (success) {
        // ✅ نجاح: نغلق الشاشة فقط (الـ AuthWrapper سيكتشف التغيير ويحولك للوحة التحكم)
        Navigator.pop(context);

        // تحديث الموقع في الخلفية
        if (authProvider.token != null) {
          _apiService.updateMyLocation(authProvider.token!, _latController.text, _lngController.text);
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
            ...
            child: Column(
              children: [
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: 'اسم المستخدم', border: OutlineInputBorder(), prefixIcon: Icon(Icons.person)),
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      Text(_locationStatus, style: TextStyle(color: _latController.text.isEmpty ? Colors.red : Colors.green, fontSize: 12, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
...
// (هذا هو كود تطبيق الزبون المعدل)
// (هذا هو الشكل الصحيح - بدون MaterialApp داخلي)
// (هذا هو الشكل الصحيح - مثل تطبيق المطعم)
class CustomerApp extends StatelessWidget {
  const CustomerApp({super.key});

  @override
  Widget build(BuildContext context) {
    // ❌ لا تضع MaterialApp هنا أبداً
    // ✅ أرجع الـ Wrapper مباشرة ليبقى متصلاً بالـ Provider
...
void main() async {
  ...
  try {
    ...
    print("⚠️ Warning: Firebase failed to initialize: $e");
  }

  runApp(
    MultiProvider(
      providers: [
        // المزود الأساسي للمصادقة
        ChangeNotifierProvider(create: (_) => AuthProvider()),

        // المزودات المعتمدة على المصادقة
        ChangeNotifierProxyProvider<AuthProvider, DashboardProvider>(
          create: (_) => DashboardProvider(),
          update: (_, auth, dash) => (dash ?? DashboardProvider())..fetchDashboardData(auth.token),
        ),
        ChangeNotifierProxyProvider<AuthProvider, StoreSettingsProvider>(
          create: (_) => StoreSettingsProvider(),
          update: (_, auth, set) => (set ?? StoreSettingsProvider())..fetchSettings(auth.token),
        ),
        ChangeNotifierProxyProvider<AuthProvider, StoreProductsProvider>(
          create: (_) => StoreProductsProvider(),
          update: (_, auth, prod) => (prod ?? StoreProductsProvider())..fetchProducts(auth.token),
        ),
      ],
      // ✅ MaterialApp واحد فقط يغلف التطبيق بأكمله
      child: MaterialApp(
        title: 'تطبيق المسواك',
        ...
        theme: ThemeData(
          ...
          appBarTheme: const AppBarTheme(
            ...
            titleTextStyle: TextStyle(color: Colors.black, fontSize: 20, fontWeight: FontWeight.bold, fontFamily: 'Tajawal'),
          ),
        ),
        // نقطة البداية هي الموجه الذكي
  ...
  Widget build(BuildContext context) {
    return Consumer<AuthProvider>(
      builder: (context, auth, child) {
        ...
        // ✅ هنا السحر: إذا تم تسجيل الدخول، اعرض لوحة التحكم فوراً
        if (auth.isLoggedIn) {
          return const StoreDashboardScreen();
        }

        // ❌ وإلا، اعرض تطبيق الزبون
