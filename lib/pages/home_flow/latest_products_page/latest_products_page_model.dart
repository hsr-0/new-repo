...
import '/index.dart';
import 'dart:async';
import 'latest_products_page_widget.dart' show LatestProductsPageWidget;
...
class LatestProductsPageModel
    extends FlutterFlowModel<LatestProductsPageWidget> {
  ...
  PagingController<ApiPagingParams, dynamic> _createGridViewController(
    ...
    return controller..addPageRequestListener(gridViewLatestProductsPage);
  }

  void gridViewLatestProductsPage(ApiPagingParams nextPageMarker) =>
      gridViewApiCall!(nextPageMarker).then((gridViewLatestProductsResponse) {
        final pageItems = (PlantShopGroup.latestProductsCall.latestProductsList(
                  gridViewLatestProductsResponse.jsonBody,
                )! ??
                [])
            .toList() as List;
        ...
        gridViewPagingController?.appendPage(
          ...
          (pageItems.length > 0)
              ? ApiPagingParams(
                  ...
                  lastResponse: gridViewLatestProductsResponse,
                )
              : null,
        );
      });
